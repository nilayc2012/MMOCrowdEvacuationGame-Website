	...
	if($result->num_rows >=1)
	{
		...
		$sql="SELECT gameid,gameplayid,playerid";
			...
			if($result->num_rows >=1)
			{
				...
				array_to_xml($report_list,$xml_data);

				$result = $xml_data->asXML('/tmp/GameData.xml');

    				header('Content-Description: File Transfer');
    				header('Content-Type: application/octet-stream');
    				header('Content-Disposition: attachment; filename="GameData.xml"');
    				header('Expires: 0');
    				header('Cache-Control: must-revalidate');
    				header('Pragma: public');
    				header('Content-Length: ' . filesize('/tmp/GameData.xml'));
    				readfile('/tmp/GameData.xml');
				unlink('/tmp/GameData.xml');
